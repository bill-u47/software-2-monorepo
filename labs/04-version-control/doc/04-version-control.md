...
- [Lab 4: Version Control](#lab-4-version-control)
  - [Objective](#objective)
  - [Setting up Git](#setting-up-git)
    - [Installing GitHub Desktop (all team members)](#installing-github-desktop-all-team-members)
    - [Creating a Git Repo (Team Member A)](#creating-a-git-repo-team-member-a)
    - [Sharing Your Repo (Team Member A)](#sharing-your-repo-team-member-a)
    - [Verifying That You Have Git (Team Member B)](#verifying-that-you-have-git-team-member-b)
    - [Cloning the Repo (Team Member B)](#cloning-the-repo-team-member-b)
  - [Working Together](#working-together)
    - [Implement Enqueue (pick a teammate to do this)](#implement-enqueue-pick-a-teammate-to-do-this)
    - [Implement Dequeue (pick another teammate to do this)](#implement-dequeue-pick-another-teammate-to-do-this)
    - [Getting Latest Changes (go back to first teammate)](#getting-latest-changes-go-back-to-first-teammate)
    - [Causing a Conflict (both teammates)](#causing-a-conflict-both-teammates)
    - [Verifying the Code](#verifying-the-code)
  - [Bonus: a Better Workflow](#bonus-a-better-workflow)
  - [Bonus: Git Commandline](#bonus-git-commandline)
  - [Troubleshooting](#troubleshooting)
    - [Cloning Script Does Not Work](#cloning-script-does-not-work)
...
### Creating a Git Repo (<ins>Team Member A</ins>)
...
### Sharing Your Repo (<ins>Team Member A</ins>)
...
### Verifying That You Have Git (<ins>Team Member B</ins>)

Now it's the other team member(s) turn. Before you can do anything, you will
need to make sure you have access to git on the command line. Unfortunately,
GitHub Desktop may not install git on your system. As a result, there are a few
things you may need to do first. For example, start by checking that git is
installed via **the VS Code terminal**:
...
### Cloning the Repo (<ins>Team Member B</ins>)

Once you have git setup, the next step is to clone the repo to your computer.
Again, there are a lot of ways to do this, but the easiest way is to use GitHub
...
To do that, you will need to get the repo URL from GitHub. You can find it
easily on the homepage by clicking on the green `<> Code` button as shown in the
screenshot below:

![GitHub URL](../assets/github-url.png)

Alternatively, you can copy the homepage URL and add `.git` to the end. In
either case, you will need the URL handy to begin cloning.

Next, you only need to copy the following command into **the VS Code terminal**
verbatim:

```sh
./labs/04-version-control/scripts/clone-repo.sh
```

If this command does not work for whatever reason, reach out to a TA or try your
hand at [the troubleshooting steps](#cloning-script-does-not-work) below.
Otherwise, you're basically done. However, you will need to add the repo you
just setup to GitHub Desktop. To do that, open GitHub Desktop. Then, open File
and select "Add local repository" or <kbd>CTRL</kbd> + <kbd>O</kbd>. If done
correctly, you should see something like this:

![Add a Local Repo](../assets/add-local-repo-cloner.png)

From there, specify the path to your software-2-monorepo by either clicking
...
> section. This is common and likely due to differing line endings between
> systems (e.g., you have a PC and your partner has a Mac). While it is likely
> safe to right click the "X changed files" line and select "Discard all
> changes", you should reach out to a TA to confirm that you aren't going
> to delete anything important.
...
Below you will find possible workarounds for issues you may run into above.

### Cloning Script Does Not Work

If the cloning script does not work on your system, you may try running the
commands within it by hand. Here is what they are and what they do. However,
please read the caution message before running them.

> [!CAUTION]
> The first step clones the repo. Please note that the URL provided is not your
> repo URL. It's a placeholder. You must replace it with your URL.
>
> The second step is going to move the contents of the repo you cloned over top
> of your existing repo. Please make sure you are doing this in the VS Code
> terminal because the default working directory should be the repo root.
>
> The last step is perhaps the most dangerous step because it contains the
> dreaded `rm -rf` command. All this does is delete a folder and all of its
> contents, but you can imagine how that might not be desirable. In this case,
> we're just deleting the tmp folder we made to house the repo.
